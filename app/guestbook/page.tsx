...
export default function GuestbookPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!error) {
      setName('');
      setMessage('');
      fetchEntries(); // 저장 후 목록 새로고침
    }
  };

  return (
    <main className="min-h-screen bg-white text-[#333] font-sans pb-20">
      <div className="max-w-2xl mx-auto px-6 pt-32">
        ...
        {/* 메시지 작성 폼 */}
        <form onSubmit={handleSubmit} className="mb-20 space-y-4 bg-gray-50 p-8 rounded-sm border border-gray-100">
          <div>
            <input
              type="text"
              placeholder="Name"
              ...
              className="w-full bg-transparent border-b border-gray-200 py-2 text-sm focus:outline-none focus:border-black transition"
            />
          </div>
          <div>
            <textarea
              ...
              className="w-full bg-transparent border-b border-gray-200 py-2 text-sm focus:outline-none focus:border-black transition h-24 resize-none"
            />
          </div>
          <button 
            type="submit" 
            className="w-full py-3 bg-black text-white text-[11px] tracking-widest uppercase hover:bg-gray-800 transition"
          >
            Leave a message
          </button>
        </form>
